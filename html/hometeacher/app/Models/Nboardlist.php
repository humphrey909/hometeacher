...
class Nboardlist extends ManageModel {
    
    

    protected $table      = 'nboard';
    ...
    protected $allowedFields = ['idx', 'uid', 'maincategorey', 'subcategorey', 'title', 'document', 'likenum', 'views', 'regdate'];
	...
	public function getlist_($conditionarray, $limit, $offset, $order) {//
        ...
        //nid로 데이터 불러옴
        public function getdataone($nid){
            //log_message('alert', json_encode($uid));
            $this->builder->where('idx', $nid);
            $query = $this->builder->get();
