...
class MyclassChatReadChklist extends ManageModel {
    
    

    protected $table      = 'myclasschatreadchklist';
    ...
    protected $allowedFields = ['idx', 'chatidx', 'rid', 'uid', 'readchk'];
    ...
    //전체 갯수 불러오기
    public function getlistcount($chatidx) {

        $this->builder->where('chatidx', $chatidx);
    ...
    //삭제 조건이 여러개일때
    public function getdelete_condition($condition) {
        log_message('alert', "---------condition--------- ".json_encode($condition));
        foreach($condition as $key => $value){
        
            //log_message('alert', "---------condition--------- ".json_encode($condition[$key]['chatidx']));
            //log_message('alert', "---------condition--------- ".json_encode($condition[$key]['rid']));
            //log_message('alert', "---------condition--------- ".json_encode($condition[$key]['uid']));
            
            $this->builder->where('chatidx', $condition[$key]['chatidx']);
            $this->builder->where('rid', $condition[$key]['rid']);
            $this->builder->where('uid', $condition[$key]['uid']);
        }
         $results = $this->builder->delete();

        // log_message('alert', "---------results--------- ".json_encode($results));
