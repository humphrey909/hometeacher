...
use App\Models\Conferencejoinuserlist;
use App\Models\MyclassRoomlistset;
use CodeIgniter\I18n\Time;
use App\Models\User;

class Conferencejoinuser extends BaseController
{
    public $oConferencejoinuserlist = "";
    public $oUser = "";
	...
	public function __construct()
    {
        ...
        $this->oConferencejoinuserlist = new Conferencejoinuserlist();
        $this->oUser = new User();
    ...
    //참여한 유저 리스트를 가져오는 함수
    public function joinuserlist($glimit, $goffset){ //uid만 join
        log_message('alert', "-------joinuserlist------------".json_encode($_POST)); 
        log_message('alert', "-------joinuserlist------------".json_encode($_GET)); 
        log_message('alert', "-------joinuserlist------------".json_encode($glimit)); 
        log_message('alert', "-------joinuserlist------------".json_encode($goffset)); 
        ...
        if($_POST['rid'] != null){
            ...
            $conditionarr["where"]["a.rid"]["value"] = $_POST['rid']; //방 idx
        }

        // if($_POST['rtcuid'] != null){
        ...
        $joinuserlist = $this->oConferencejoinuserlist->getlist_join($conditionarr, $pagehandler);
	    


        //재정렬해서 내것이 맨위에 오도록 할 것.
        if($_POST['myuid'] != null){
            $newlist_order = array();
            for($i = 0; $i<count($joinuserlist);$i++){
                

                if($joinuserlist[$i]->uid == $_POST['myuid']){ //통과

                    //log_message('alert', "-------joinuserlist uid------------".json_encode($joinuserlist[$i]->uid)); 
                    $removeArray = array_splice($joinuserlist,$i,1); //내것 삭제

                    log_message('alert', "-------joinuserlist removeArray------------".json_encode($removeArray)); 
                    log_message('alert', "-------joinuserlist joinuserlist------------".json_encode($joinuserlist)); 

                    array_push($newlist_order, $removeArray[0]);
                }
            }

            //내거 뺀값을 다시 넣어줌
            for($j = 0; $j<count($joinuserlist);$j++){
                array_push($newlist_order, $joinuserlist[$j]);
            }

            log_message('alert', "-------total joinuserlist------------".json_encode($joinuserlist)); 
            log_message('alert', "-------total joinuserlist neworder------------".json_encode($newlist_order)); 

            $response = $this->response->setJSON($newlist_order);
        }else{
            $response = $this->response->setJSON($joinuserlist);
        }
	    return $response;
    }
    ...
    public function checkuserinroom($glimit, $goffset){ //uid만 join
        log_message('alert', "-------joinuserlist------------".json_encode($_POST)); 
        log_message('alert', "-------joinuserlist------------".json_encode($_GET)); 
        log_message('alert', "-------joinuserlist------------".json_encode($glimit)); 
        log_message('alert', "-------joinuserlist------------".json_encode($goffset)); 
    ...
    public function joinuser($a,$b){
        ...
        if($_POST != null){
            ...
            //저장되어 있는지 체크 후 저장되어있지 않으면 저장
            //이미 있으면 업데이트 처리한다. 

            //회의 참여자 리스트에서 참여자 있는지 체크
            $conditionarr = array();
            if($_POST['uid'] != null || $_POST['rid'] != null){
                $conditionarr["where"]["uid"]["logic"] = "=";
                $conditionarr["where"]["uid"]["value"] = $_POST['uid'];
                $conditionarr["where"]["rid"]["logic"] = "=";
                $conditionarr["where"]["rid"]["value"] = $_POST['rid'];
            }
            $pagehandler = array();
            $pagehandler["limit"] = 0; //리스트 갯수
            $pagehandler["offset"] = 0; //리스트 위치 
    
            log_message('alert', "Conferencejoinuserlist condition  ".json_encode($conditionarr));
            $joinuserlist = $this->oConferencejoinuserlist->getlist($conditionarr, $pagehandler);


            if(!empty($joinuserlist)){//존재o 업데이트
                //회의 참여자 업데이트
                $results = $this->getdataupdate($_POST, $joinuserlist[0]->idx);
            }else{//존재X 저장
                //회의 참여자 추가
            ...
            //idx를 가져오고 그 idx로 정보를 가져올 것. 
            $resultsid = $this->oConferencejoinuserlist->getinsertid();
            
            log_message('alert', "resultsid-----------  ".json_encode($resultsid));  
            ...
            if($resultsid != null){
                $conditionarr["where"]["a.idx"]["logic"] = "=";
                $conditionarr["where"]["a.idx"]["value"] = $resultsid; //방 idx
            }
           ...
           // $userresponse = $this->response->setJSON($joinuserlist);



            log_message('alert', "resultsid-----------  ".json_encode($joinuserlist));  
           // log_message('alert', "resultsid-----------  ".json_encode($userresponse));  
            ...
            if($results == 1){
                $response = $this->response->setJSON(['result'  => true,  'err'  => "success", 'userdata' => $joinuserlist]);
                log_message('alert', "회의 참여자 생성 에러사항-----------  ".json_encode(true));  
            }else{
                $response = $this->response->setJSON(['result'  => false, 'err'  => "false"]);
                log_message('alert', "회의 참여자 생성 에러사항-----------  ".json_encode("no create room"));  
            }
    ...
    public function deleteuser($a,$b){
        ...
        if($_POST != null){
            ...
            if($_POST['rid'] != null){
                ...
                $conditionarr["where"]["uid"]["logic"] = "=";
                $conditionarr["where"]["uid"]["value"] = $_POST['uid']; //유저 idx
            }
            $pagehandler = array();
            $pagehandler["limit"] = 0; //리스트 갯수
            $pagehandler["offset"] = 0; //리스트 위치 
            $joinuserinfo = $this->oConferencejoinuserlist->getlist($conditionarr, $pagehandler);


            //방 삭제
            $deletelist = array();
            $deletelist[0] = $joinuserinfo[0]->idx;
            $response = $this->oConferencejoinuserlist->getdelete("idx", $deletelist);
