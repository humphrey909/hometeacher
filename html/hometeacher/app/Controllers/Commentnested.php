...
use App\Models\Likecommentnestedlist;
use CodeIgniter\I18n\Time;
use App\Models\User;
use App\Models\CommentnestedImglist;
use App\Models\Alertlist;

class Commentnested extends BaseController
{
    public $oCommentlistNested = "";
    public $oLikecommentnestedlist = "";
    public $now = "";
    public $oCommentnestedImglist = "";
    public $oAlertlist = "";
	...
	public function __construct()
    {
        ...
        $this->oLikecommentnestedlist = new Likecommentnestedlist();
        $this->oCommentnestedImglist = new CommentnestedImglist();
    ...
    public function commentlist($glimit, $goffset){ //uid만 join
        ...
        $conditionarr = array();
        $conditionarr["feild"] = "a.idx,a.nid,a.cid,a.uid,a.basicuri,a.src,a.document,a.likenum,a.regdate,b.usertype,b.email,b.name,b.nicname, c.idx as imgidx, c.basicuri as profilebasicuri, c.src as profilesrc, c.type";
        
        //댓글고유번호
        if($_POST['cnid'] != null){
            $conditionarr["where"]["a.idx"]["logic"] = "=";
            $conditionarr["where"]["a.idx"]["value"] = $_POST['cnid'];
        }

        //댓글고유번호
        if($_POST['cid'] != null){
            $conditionarr["where"]["a.cid"]["logic"] = "=";
            $conditionarr["where"]["a.cid"]["value"] = $_POST['cid'];
        }

        //게시글 고유번호
        if($_POST['nid'] != null){
            $conditionarr["where"]["a.nid"]["logic"] = "=";
            $conditionarr["where"]["a.nid"]["value"] = $_POST['nid'];
        }
        ...
        $commentnestedlist = $this->oCommentlistNested->getlist_join($conditionarr, $pagehandler);
        ...
        for($i = 0; $i<count($commentnestedlist);$i++){
            ...
            if($_POST['uid'] != null){
                ...
                $conditionarr["where"]["getcnid"]["logic"] = "=";
                $conditionarr["where"]["getcnid"]["value"] = $commentnestedlist[$i]->idx;

                $pagehandler = array();
                $pagehandler["limit"] = 0; //리스트 갯수
                $pagehandler["offset"] = 0; //리스트 위치 
                $commentlikeinfo = $this->oLikecommentnestedlist->getlist($conditionarr, $pagehandler);
                log_message('alert', "댓글 좋아요 데이터 ".json_encode($commentlikeinfo));
            
                if(!empty($commentlikeinfo)){ //like 리스트가 있을때만 푸쉬함
                    $commentnestedlist[$i]->commentlikechk = "true";
                }else{
                    $commentnestedlist[$i]->commentlikechk = "false";
                }
            }
            ...
            $pagehandler["limit"] = 0; //리스트 갯수
            $pagehandler["offset"] = 0; //리스트 위치 
            $commentlikenum = $this->oLikecommentnestedlist->getlist($conditionarr, $pagehandler);
            log_message('alert', "댓글 좋아요 갯수 ".json_encode($commentlikenum));
        
            if(!empty($commentlikenum)){ //like 리스트가 있을때만 푸쉬함
                $commentnestedlist[$i]->commentliketotalnum = count($commentlikenum);
             }else{
                $commentnestedlist[$i]->commentliketotalnum = 0;
            }

            //대댓글 이미지 추가
            $commentimg = $this->commentimglist($commentnestedlist[$i]->idx);
            ...
            if(!empty($commentimg)){ //이미지 데이터가 있는 게시물만 푸쉬함
               ...
               $commentnestedlist[$i]->commentimg = $commentimg;
            }else{
                $commentnestedlist[$i]->commentimg = "null";
            }

        }

        
	    log_message('alert', json_encode($commentnestedlist));
	    $response = $this->response->setJSON($commentnestedlist);
    ...
    //댓글 이미지 리스트 
    public function commentimglist($cnid){


        $conditionarr = array();

        $conditionarr["where"]["cnid"]["logic"] = "=";
        $conditionarr["where"]["cnid"]["value"] = $cnid; 
        ...
        $results = $this->oCommentnestedImglist->getlist($conditionarr, $pagehandler);

        //log_message('alert', "------------oCommentnestedImglist----------".json_encode($results));
    ...
    public function commentlist_mypage($glimit, $goffset){
        ...
        $commentlist = $this->oCommentlistNested->getlist_join_mypage($conditionarr, $pagehandler);

        log_message('alert', "commentNestedlist-----------  ".json_encode($commentlist));  
    ...
    public function commentsavebox(){
        ...
        if($_POST != null){
            ...
            if($results == 1){
                //방금 insert한 id 가져오기
                $resultsid = $this->oCommentlistNested->getinsertid();
        ...
        if($_FILES != null){
            ...
            $fileresult = $this->imgupload($_FILES, $resultsid, $_POST['cid'], $_POST['uid'], $_POST['currenttime']);
        ...
        $data=array("idx" => '0', 'nid' => $_POST["nid"], 'uid' => $_POST["uid"], 'alertuid' => $_POST["Commentwriteuid"], 'alertdocu' => $_POST["myname"]."님이 회원님의 댓글에 대댓글을 작성하였습니다. ", 'click' => 1, 'regdate' => $_POST['currenttime']);
    ...
    public function commentupdatebox(){
        ...
        if($_FILES != null){
            ...
            $fileresult = $this->imgupload($_FILES, $_POST['cnid'], $_POST['cid'], $_POST['uid'], $_POST['currenttime']);

           // $fileresult = $this->imgupload($_FILES, $commentidx);
    ...
    public function deleteimage($deletelist){
        ...
        $list = $this->oCommentnestedImglist->getdelete("idx", $deletelistarr);
    }
    ...
    public function getdatainsert($Data){
        ...
        $data=array("idx" => '0', 'nid' => $Data["nid"], 'cid' => $Data["cid"], 'uid' => $Data["uid"], 'basicuri' => "", 'src' => "", 'document' => $Data["document"], 'likenum' => 0, 'regdate' => $Data["currenttime"]);

        $results = $this->oCommentlistNested->getinsert($data);
    ...
    public function getdataupdate($Data, $File){
        ...
        if($File == null){
            $data=array('nid' => $Data["nid"], 'cid' => $Data["cid"], 'uid' => $Data["uid"], 'basicuri' => "", 'src' => "", 'document' => $Data["document"], 'regdate' => $Data["currenttime"]);
        }else{
            $data=array('nid' => $Data["nid"], 'cid' => $Data["cid"], 'uid' => $Data["uid"], 'document' => $Data["document"], 'regdate' => $Data["currenttime"]);
        ...
        $results = $this->oCommentlistNested->getupdate($fieldname, $data);
    ...
    public function getdataupdate_each(){
        ...
        if($json->password != null){ //비회원
           ...
           $chkdata = $this->oCommentlistNested->Chkpassword($json->idx, $json->password);
    ...
    public function getdatadelete(){
        ...
        //대댓글 삭제
        $deletelist = array();
        $deletelist[0] = $_POST['cnid'];
        //log_message('alert', "-----getdatadelete-----".json_encode($deletelist));
        $results = $this->oCommentlistNested->getdelete("idx", $deletelist);

        //해당 이미지 삭제 
        // $fieldname=array(
        //     'idx' => $json->idx
        // );
        $list = $this->oCommentnestedImglist->getdelete("cnid", $deletelist);
    ...
    public function imgupload($files, $cnid, $cid, $uid, $currenttime){
        ...
        foreach($files as $key => $value){
            ...
            $imgdata=array("idx" => '0', 'cid' => $cid, 'cnid' => $cnid, 'uid' => $uid, 'basicuri' => $basicurl, 'src' => $filename, 'regdate' => $currenttime);

            //이미지 경로 db 저장
            $reslut = $this->oCommentnestedImglist->getinsert($imgdata);
    ...
    public function likeadd($a, $b){
        ...
        $conditionarr["where"]["giveuid"]["value"] = $_POST["uid"];
        $conditionarr["where"]["getcnid"]["logic"] = "=";
        $conditionarr["where"]["getcnid"]["value"] = $_POST["cnid"];

        $pagehandler = array();
        $pagehandler["limit"] = 0; //리스트 갯수
        $pagehandler["offset"] = 0; //리스트 위치 
        $likeinfo = $this->oLikecommentnestedlist->getlist($conditionarr, $pagehandler);
        ...
        if($_POST["liketype"] == 1){ //좋아요 클릭
            if($likeinfo[0] == ""){ //값이 없을때만 
                $data=array("idx" => '0', 'giveuid' => $_POST["uid"], 'getcnid' => $_POST["cnid"], 'regdate' => $_POST['currenttime']);
                log_message('alert', "프로필 데이터 ".json_encode($data));
        
                $results = $this->oLikecommentnestedlist->getinsert($data);
            }else{
                $results = false;
            }

            //알림 저장하기.
            $data=array("idx" => '0', 'nid' => $_POST["nid"], 'uid' => $_POST["uid"], 'alertuid' => $_POST["Commentnestedwriteuid"], 'alertdocu' => $_POST["myname"]."님이 회원님의 대댓글에 좋아요를 눌렀습니다. ", 'click' => 1, 'regdate' => $_POST['currenttime']);
            log_message('alert', "알림데이터 데이터 ".json_encode($data));
            ...
            if($likeinfo[0] != ""){ //값이 있으면
                ...
                $results = $this->oLikecommentnestedlist->getdelete("idx", $deletearr);
            }else{
                $results = false;
            }
        }
