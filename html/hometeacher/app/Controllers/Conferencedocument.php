...
use App\Models\Conferencedocumentlist;
use App\Models\MyclassRoomlistset;
use CodeIgniter\I18n\Time;
use App\Models\User;

class Conferencedocument extends BaseController
{
    public $oConferencedocumentlist = "";
    public $oUser = "";
	...
	public function __construct()
    {
        ...
        $this->oConferencedocumentlist = new Conferencedocumentlist();
        $this->oUser = new User();
    ...
    public function documentlist($glimit, $goffset){ //uid만 join
        ...
        $documentlist = $this->oConferencedocumentlist->getlist($conditionarr, $pagehandler);
	    
       

	    
        log_message('alert', "-------total vodlist------------".json_encode($documentlist)); 
	    $response = $this->response->setJSON($documentlist);
    ...
    public function savebox($a,$b){
        ...
        log_message('alert', json_encode("--------------------------")); 
        log_message('alert', json_encode("--------------------------")); 
        log_message('alert', json_encode("--------------pdf get0------------")); 
        
        //log_message('alert', json_encode("--------------objcount------------")); 
        //log_message('alert', json_encode($_POST['objcount'])); 
        //log_message('alert', json_encode("--------------objcount------------")); 
         ...
         //post전부 저장 
        if($_POST != null){
            //docutype : 1. 빈페이지 생성, 2. 이미지생성, 3. pdf파일 생성
            if($_POST['docutype'] == '1'){ //빈페이지
                if($_POST['firsttype'] == "0" || $_POST['firsttype'] == "2"){ //화이트보드에 접근시 : 데이터가 있는지 확인 후 없으면 저장, 있으면 저장안함.
                    $conditionarr = array();
                    ...
                    $documentlist_exchk = $this->oConferencedocumentlist->getlist($conditionarr, $pagehandler);
                    if(empty($documentlist_exchk)){ //데이터 없으면 저장 : 처음 문서가 무조건 존재해야함.
                        $results = $this->getdatainsert($_POST);

                        if($results == 1){
                            $resultsid = $this->oConferencedocumentlist->getinsertid();
                            ...
                            $documentlist = $this->oConferencedocumentlist->getlist($conditionarr, $pagehandler);
                            log_message('alert', "  ------response-----  ".json_encode($documentlist));
        
                            $response = $this->response->setJSON($documentlist);
                            
                        }else{
                            $response = $this->response->setJSON([['result'  => false, 'err'  => "false"]]);
                            log_message('alert', "회의 문서 저장 에러사항-----------  ".json_encode("no create room")); 
                        }

                    }else{ //값이 있을때 : 1개이상의 문서가 존재할때, 페이징 처리가 필요하다. 
                        //$response = $this->response->setJSON([['result'  => false, 'err'  => "false"]]);
                        //log_message('alert', "회의 문서 저장 에러사항-----------  ".json_encode("no create room")); 
                        ...
                        $pagehandler = array();
                        //$pagehandler["limit"] = $_POST['objcount']; //리스트 갯수
                        $pagehandler["limit"] = 0; //리스트 갯수
                        $pagehandler["offset"] = 0; //리스트 위치 
            ...
            }else if($_POST['docutype'] == '2'){ //이미지 파일
                if($_POST['firsttype'] == "1"){
            ...
            }else if($_POST['docutype'] == '3'){ //pdf 파일
                log_message('alert', json_encode("--------------pdf get1------------")); 
                if($_POST['firsttype'] == "1"){
                    log_message('alert', json_encode("--------------pdf get2------------")); 
                    if($_FILES != null){
                        log_message('alert', json_encode("--------------pdf get3------------")); 

                        log_message('alert', "  ------pdf files-----  ".json_encode($_FILES));

                       // $fileresult = $this->imgupload($_FILES, $resultsid);
    ...
    //문서 삭제
    public function deletebox($a,$b){
        log_message('alert', json_encode("--------------------------")); 
        log_message('alert', json_encode("--------------------------")); 
        log_message('alert', json_encode($_POST)); 
        log_message('alert', json_encode("--------------------------")); 
        log_message('alert', json_encode("--------------------------")); 
        log_message('alert', "문서 삭제".json_encode($_POST['DeleteDocumentlist'])); 

        $deletelistDecode = json_decode($_POST['DeleteDocumentlist']);
        //log_message('alert', json_encode(count($deletelist))); 

        //post
        if($_POST != null){
            for($i = 0;$i<count($deletelistDecode);$i++){
                //log_message('alert', $deletelistDecode[$i]); 

                //방 삭제
                $deletelist = array();
                $deletelist[0] = $deletelistDecode[$i];
                //log_message('alert', "-----getdatadelete-----".json_encode($deletelist));
                $delresult = $this->oConferencedocumentlist->getdelete("idx", $deletelist);
    ...
    public function getdatainsert($Data){
        ...
        $data=array("idx" => '0', 'uid' => $Data["uid"], 'rid' => $Data["rid"], 'docutype' => $Data["docutype"], 'basicuri' => "", 'src' => "", 'regdate' => $Data["currenttime"]);

        $results = $this->oConferencedocumentlist->getinsert($data);
    ...
    public function getdataupdate($Data, $joinidx){
        ...
        $results = $this->oConferencedocumentlist->getupdate($fieldname, $data);
    ...
    public function imgupload($files, $Data){
        ...
        $basicurl = "/image/conferencedocumentimg/";
        ...
        foreach($files as $key => $value){
            ...
            $imgdata=array("idx" => '0', 'uid' => $Data["uid"], 'rid' => $Data["rid"], 'docutype' => $Data["docutype"], 'basicuri' => $basicurl, 'src' => $filename, 'regdate' => $Data["currenttime"]);

            //이미지 경로 db 업데이트
            $reslut = $this->oConferencedocumentlist->getinsert($imgdata);
