...
class Classrequestchat extends BaseController
{
    ...
    public function chatlist($glimit, $goffset){ //uid만 join
        ...
        $conditionarr = array();
        $conditionarr["feild"] = "a.idx,a.rid,a.uid,a.type,a.message, a.imgchk, a.rid_myclass, a.availablilty, a.regdate,b.usertype,b.email,b.name,b.nicname, c.idx as imgidx, c.basicuri as profilebasicuri, c.src as profilesrc, c.type as profiletype";
        ...
        $classrequestlist = $this->oClassRequestChatlist->getlist_join($conditionarr, $pagehandler);
    ...
    public function classrequestroominfo($glimit, $goffset){
        ...
        if($_POST['roomidx'] != null){
         ...
         $classrequestinfo = $this->oClassRequestChatlist->getlist($conditionarr, $pagehandler);
    ...
    public function makeroom($a,$b){
        ...
        if($_POST != null){
            ...
            $roomchk = $this->oClassRequestChatlist->getlist($conditionarr, $pagehandler);
            ...
            if(count($roomchk) == 0){ 
                ...
                if($results == 1){
                    ...
                    //방금 insert한 id 가져오기
                    $resultsid = $this->oClassRequestChatlist->getinsertid();
    ...
    public function getdatainsert($Data){
        ...
        $roomlist = $this->oClassRequestChatlist->getlist($conditionarr, $pagehandler);
        ...
        $results = $this->oClassRequestChatlist->getinsert($data);
    ...
    public function getdataupdate($Data){
        ...
        $results = $this->oClassRequestChatlist->getupdate($fieldname, $data);
    ...
    public function deletemsg($a,$b){
        ...
        if($_POST != null){
            ...
            $response = $this->oClassRequestChatlist->getdelete("rid", $deletelist);
    ...
    public function setquestionmessage(){
        log_message('alert', json_encode("--------------------------")); 
        log_message('alert', json_encode("--------------------------")); 
        log_message('alert', json_encode($_POST)); 
        log_message('alert', json_encode("--------------------------")); 
        ...
        //1. 질문 메세지의 효용여부를 체크한다. 한번 클릭시 0으로 변경한다. 

        //availablilty으로 변경
        $fieldname=array(
            'idx' => $_POST["chatidx"]
        );
        if($_POST["cktype"] == "1"){ //승인
            $data=array('availablilty' => "2");
        }else{ //거절
            $data=array('availablilty' => "3");
        }
        ...
        //2. 나의 과외에 유저의 초대 여부를 수정한다. 
        $fieldname_mcuser=array(
            'rid' => $_POST["rid_myclass"],
            'uid' => $_POST["myuid"]
        );
        
        if($_POST["cktype"] == "1"){ //승인
            $data_mcuser=array('invitechk' => "2");
        }else{ //거절
            $data_mcuser=array('invitechk' => "3");
        }
        $results = $this->oMyclassUserlist->getupdate($fieldname_mcuser, $data_mcuser);

        //해당 방의 채팅 갯수 가져오기
        $chatcount = $this->oClassRequestChatlist->getlistcount($_POST["rid_myclass"]);
        //$classrequestlist[$i]->totalchatcount = $chatcount[0]->idx;
        log_message('alert', " --------------- chatcount ---------------  ".json_encode($chatcount[0]->idx));


        $results = $this->response->setJSON(['result'  => true, 'cktype' =>  $_POST["cktype"], 'rid_myclass' =>  $_POST["rid_myclass"], 'myclass_chatcount' => $chatcount[0]->idx]);
        return $results; 
    }
}
